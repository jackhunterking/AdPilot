/**
 * Feature: Pause Ad API v1
 * Purpose: Pause a live ad
 * References:
...
export async function POST(
  ...
  try {
    ...
    if (error) {
      console.error('[v1/ads/:id/pause] Error:', error)
      return NextResponse.json(
        { success: false, error: { code: 'pause_failed', message: 'Failed to pause ad' } },
        { status: 500 }
      )
    }
    ...
    console.error('[v1/ads/:id/pause] Unexpected error:', error)
    return NextResponse.json(
